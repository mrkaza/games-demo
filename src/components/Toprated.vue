<template>
  <div class="popular">
    ...
    >
        <slide class="igrica-poz" v-for="(igrica,index) in toprated" :key="index">
    ...
    <div class="navigacija">
        ...
        :mouseDrag="false"
        :touchDrag="false" :keysControl="false">
            <slide class="slikice" v-for="(igrica,index) in toprated" :key="index">
...
export default {
  ...
  created() {
    ...
    this.axios.get(`https://api.rawg.io/api/games?dates=2010-01-01,2020-${this.mjesec}-${this.dan}&ordering=-rating`).then(response => {
        this.toprated = response.data.results.slice(0,5)
